...
var account = await manager.GetAccountByAccountNumberAsync("89989");
...
else
{
  ...
  while (looping)
  {
    ...
    switch (option)
    {
      ...
      case "D":
        Transactions.DoTransaction("Deposit", account.Deposit);
        break;
      case "W":
        Transactions.DoTransaction("Withdrawal", account.Withdraw);
        break;
...
public static class Transactions
{
  ...
  public static void DoTransaction(string txPrompt, Action<decimal> txOperation )
  {
    Console.Write($"{txPrompt} Amount: ");
    var txAmountEntered = Console.ReadLine();
    if (decimal.TryParse(txAmountEntered, out var amount))
    {
      txOperation(amount);
    }
  }
